...
async function createSOSEvent(data) {
    try {
        logger_1.logger.debug('Creating SOS event with data:', {
        ...
        if (recentSOS && recentSOS.length > 0) {
            logger_1.logger.info('Duplicate SOS detected within 30s, returning existing event:', recentSOS[0].id);
                ...
                if (incomingAttachments.length > 0) {
                    ...
                    if (merged.length !== existingAttachments.length) {
                        const { data: updated, error: updateError } = await supabaseAdmin_1.supabaseAdmin
                            ...
                            .single();
                        if (updateError) {
                            logger_1.logger.warn('Failed to merge attachments into deduped SOS event:', updateError);
        ...
        const { data: event, error } = await supabaseAdmin_1.supabaseAdmin
            ...
            .single();
        if (error) {
            logger_1.logger.error('Failed to create SOS event:', error);
            throw new Error(`Failed to create SOS event: ${error.message || error.code || 'Unknown error'}`);
        }
        if (!event || !event.id) {
            logger_1.logger.error('SOS event insert returned null or missing ID');
            throw new Error('Failed to create SOS event: No event returned from database');
        }
        logger_1.logger.debug('SOS event created successfully:', event.id);
        ...
        try {
            ...
            logger_1.logger.warn('Failed to log sos_triggered event (non-critical):', logError.message);
            // Don't throw - event logging should not break SOS creation
        }
        return event;
    }
    catch (error) {
        logger_1.logger.error('Exception in createSOSEvent:', error);
        throw error;
    }
}
    ...
    try {
        ...
        if (error) {
            ...
            if (error.code !== 'PGRST116') {
                logger_1.logger.error('Failed to fetch SOS event by ID:', error);
                logger_1.logger.error('Event ID:', eventId);
            }
            return null;
        }
        if (!data) {
        ...
        logger_1.logger.error('Exception in getSOSEventById:', error);
        return null;
    }
}
async function getSOSEvents(filters) {
    try {
        ...
        if (error) {
            logger_1.logger.error('Failed to fetch SOS events:', error);
            logger_1.logger.debug('Filters used:', filters);
            throw new Error(`Failed to fetch SOS events: ${error.message || error.code || 'Unknown error'}`);
        }
        if (!data) {
            logger_1.logger.warn('getSOSEvents returned null data');
            return [];
        }
        return data;
    }
    catch (error) {
        logger_1.logger.error('Exception in getSOSEvents:', error);
        throw error;
    }
}
async function updateSOSStatus(eventId, status, securityId) {
    ...
    try {
        const { data: chats, error: chatFetchError } = await supabaseAdmin_1.supabaseAdmin
            ...
            .limit(1);
        if (chatFetchError) {
            logger_1.logger.warn('Failed to fetch sos_chats row (non-blocking):', chatFetchError.message);
        }
        ...
        logger_1.logger.warn('Failed to upsert/assign sos_chat (non-blocking):', e?.message || e);
    }
    let securityEmail;
    let securityName;
    try {
...
async function logSOSEvent(data) {
    try {
        ...
        if (error) {
            logger_1.logger.warn('Failed to log SOS event:', error.message, error);
            // Don't throw - event logging should not break main flow
        }
        else {
            logger_1.logger.debug(`Logged SOS event: ${data.type} for SOS ${data.sos_id}`);
        }
    }
    catch (error) {
        // Log error but don't throw - event logging should not break main flow
        logger_1.logger.warn('Failed to log SOS event (exception):', error.message, error);
    }
}
/**
...
async function getSOSEventHistory(sosId) {
    ...
    if (error) {
        logger_1.logger.error('Failed to fetch SOS event history:', error);
        throw new Error(`Failed to fetch SOS event history: ${error.message}`);
    }
    // Always return an array, even if empty
    const events = data || [];
    logger_1.logger.debug(`Retrieved ${events.length} events for SOS ${sosId}`);
    return events;
}
