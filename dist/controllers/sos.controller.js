...
const riskZonesCache_1 = require("../services/riskZonesCache");
...
const createSOS = async (req, res) => {
    try {
        ...
        logger_1.logger.debug('Creating SOS event for user', { userId: req.user.id, risk_score });
        ...
        if (!event || !event.id) {
            logger_1.logger.error('SOS event creation returned invalid event:', event);
        ...
        try {
            ...
            if (typeof msg === 'string' && /duplicate|unique/i.test(msg)) {
                ...
                logger_1.logger.warn('Failed to create sos_chats row (non-blocking):', msg);
            }
        }
        ...
        if (location && typeof location.lat === 'number' && typeof location.lng === 'number') {
            try {
                const riskZones = await (0, riskZonesCache_1.getRiskZonesCached)();
                ...
                logger_1.logger.warn('Failed to log zone_entered event:', error.message);
            }
        }
        ...
        if (io) {
            ...
            logger_1.logger.debug(`Emitted SOS event to security_room and user_${req.user?.id}`);
        }
        else {
            logger_1.logger.warn('Socket.io not available - SOS event not broadcasted');
        }
        res.status(201).json(eventWithEmail);
    }
    catch (error) {
        logger_1.logger.error('Error creating SOS event:', error);
...
const getSOS = async (req, res) => {
    try {
        ...
        logger_1.logger.debug('Fetching SOS events with filters:', filters);
        ...
        logger_1.logger.debug(`Retrieved ${events.length} SOS events`);
        res.json(events);
    }
    catch (error) {
        logger_1.logger.error('Error in getSOS:', error);
...
const getSOSById = async (req, res) => {
    try {
        ...
        const envBucket = env_1.env.supabaseStorageBucket();
