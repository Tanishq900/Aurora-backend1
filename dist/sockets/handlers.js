...
function setupSocketHandlers(io) {
    ...
    io.use(async (socket, next) => {
        try {
            ...
            if (!token) {
                logger_1.logger.warn('Socket auth failed: No token provided');
            ...
            try {
                ...
                logger_1.logger.warn('Socket auth failed: Token verification error', tokenError.message);
            ...
            try {
                ...
                logger_1.logger.warn('Socket auth failed: User lookup error', userError.message);
                return next(new Error('User lookup failed'));
            }
            if (!user) {
                logger_1.logger.warn('Socket auth failed: User not found', payload.userId);
                return next(new Error('User not found'));
            }
            if (user.role === 'security' && !user.security_approved) {
                logger_1.logger.warn('Socket auth failed: Security account pending admin approval', payload.userId);
            ...
            logger_1.logger.error('Socket auth failed: Unexpected error', error.message || error);
            next(new Error('Authentication failed'));
        }
    });
    io.on('connection', (socket) => {
        logger_1.logger.debug(`Socket connected: ${socket.userId} (${socket.userRole})`);
        // Security personnel join security room
        if (socket.userRole === 'security') {
            socket.join('security_room');
            logger_1.logger.debug(`Security user ${socket.userId} joined security_room`);
        }
        ...
        socket.on('join_sos', (sosId) => {
            ...
            logger_1.logger.debug(`Socket ${socket.userId} joined sos_${sosId}`);
        });
