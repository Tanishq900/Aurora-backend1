...
export async function createSOSEvent(data: {
  ...
  try {
    logger.debug('Creating SOS event with data:', {
    ...
    if (recentSOS && recentSOS.length > 0) {
      logger.info('Duplicate SOS detected within 30s, returning existing event:', recentSOS[0].id);
      ...
      if (existingEvent) {
        ...
        if (incomingAttachments.length > 0) {
          ...
          if (merged.length !== existingAttachments.length) {
            ...
            if (updateError) {
              logger.warn('Failed to merge attachments into deduped SOS event:', updateError);
    ...
    if (error) {
      logger.error('Failed to create SOS event:', error);
      throw new Error(`Failed to create SOS event: ${error.message || error.code || 'Unknown error'}`);
    }

    if (!event || !event.id) {
      logger.error('SOS event insert returned null or missing ID');
    ...
    try {
      ...
      logger.warn('Failed to log sos_triggered event (non-critical):', logError.message);
      // Don't throw - event logging should not break SOS creation
    }

    return event;
  } catch (error: any) {
    logger.error('Exception in createSOSEvent:', error);
    throw error;
  }
}
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code !== 'PGRST116') {
        logger.error('Failed to fetch SOS event by ID:', error);
    ...
    logger.error('Exception in getSOSEventById:', error);
    return null;
  }
}
...
export async function getSOSEvents(filters?: {
  ...
  try {
    ...
    if (error) {
      logger.error('Failed to fetch SOS events:', error);
      logger.debug('Filters used:', filters);
      throw new Error(`Failed to fetch SOS events: ${error.message || error.code || 'Unknown error'}`);
    }

    if (!data) {
      logger.warn('getSOSEvents returned null data');
      return [];
    }

    return data;
  } catch (error: any) {
    logger.error('Exception in getSOSEvents:', error);
    throw error;
  }
}
...
export async function updateSOSStatus(
  ...
  try {
    ...
    if (chatFetchError) {
      logger.warn('Failed to fetch sos_chats row (non-blocking):', chatFetchError.message);
    }
    ...
    logger.warn('Failed to upsert/assign sos_chat (non-blocking):', e?.message || e);
  }
...
export async function logSOSEvent(data: {
  ...
  try {
    ...
    if (error) {
      logger.warn('Failed to log SOS event:', error.message, error);
      // Don't throw - event logging should not break main flow
    } else {
      logger.debug(`Logged SOS event: ${data.type} for SOS ${data.sos_id}`);
    }
  } catch (error: any) {
    // Log error but don't throw - event logging should not break main flow
    logger.warn('Failed to log SOS event (exception):', error.message, error);
  }
}
...
export async function getSOSEventHistory(sosId: string): Promise<Array<{
  ...
  if (error) {
    logger.error('Failed to fetch SOS event history:', error);
    throw new Error(`Failed to fetch SOS event history: ${error.message}`);
  }

  // Always return an array, even if empty
  const events = data || [];
  logger.debug(`Retrieved ${events.length} events for SOS ${sosId}`);
  return events;
}
